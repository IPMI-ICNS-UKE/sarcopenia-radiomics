...
def process_map(map_name: str) -> pd.DataFrame:
    print("=" * 80)
    print(f"Processing deep radiomics map: {map_name}")
    ...
    df_c2 = align_to_reference_features(df_c1, df_c2, id_columns, label=f"deep/{map_name}/cohort2")
    ...
    result = run_analysis_only_pipeline(
        ...
        rho_threshold=RCFG.spearman_rho_threshold_qc,
        dataset_name=map_name,
    )
    result_c2 = run_analysis_only_pipeline(
        df_all=df_c2,
        feature_cols=feature_cols,
        id_columns=id_columns,
        rho_threshold=RCFG.spearman_rho_threshold_qc,
    ...
    print(f"Stable/retained embedding dimensions: {len(feature_cols)}")
