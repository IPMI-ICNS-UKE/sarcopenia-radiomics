...
def process_map(map_name: str) -> pd.DataFrame:
    ...
    df_c2 = align_to_reference_features(df_c1, df_c2, id_columns, label=f"spectral/{map_name}/cohort2")
    ...
    df_filtered_c2 = subset_with_ids(df_c2, id_columns, result.selected_features).reset_index(drop=True)

    result.df_all.to_csv(out_dir / RCFG.filtered_features_pattern.format(map_name=map_name), index=False)
    df_filtered_c2.to_csv(out_dir / RCFG.filtered_features_pattern_cohort_2.format(map_name=map_name), index=False)

    result.variance_summary.to_csv(tables_dir / f"near_zero_variance_summary_{map_name}.csv", index=False)
    result.correlation_summary.to_csv(tables_dir / f"correlation_clustering_summary_{map_name}.csv", index=False)
    result.overall_summary.to_csv(tables_dir / f"filtering_overall_summary_{map_name}.csv", index=False)
    ...
    if not result.corr_matrix_train.empty:
        result.corr_matrix_train.to_csv(tables_dir / f"spearman_corr_matrix_train_after_nzv_{map_name}.csv")

    if RCFG.make_plots:
        row = result.overall_summary.iloc[0]
        plot_feature_counts(
            [int(row["n_initial_stable_features"]), int(row["n_after_nzv"]), int(row["n_after_correlation_filtering"])],
...
def main() -> None:
    ...
    if summaries:
        ...
        pd.concat(summaries, axis=0, ignore_index=True).to_csv(RCFG.output_root / "filtering_summary_all_maps.csv", index=False)
