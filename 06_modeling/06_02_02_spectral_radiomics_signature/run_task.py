...
from models import build_group_lookup, get_pass1_specs, get_pass2_specs
...
# Merge two CV output dicts
def _merge_outputs(o1: Dict, o2: Dict) -> Dict:
    concat_keys = [
        "cv_fold_metrics", "cv_summary", "oof_predictions", "oof_predictions_aggregated",
        "inner_tuning_cv", "test_metrics", "test_predictions", "coefficients",
        "inner_tuning_refit", "model_info", "errors",
    ]
    merged: Dict = {}
    for key in concat_keys:
        frames = [d.get(key, pd.DataFrame()) for d in (o1, o2)]
        non_empty = [f for f in frames if isinstance(f, pd.DataFrame) and not f.empty]
        merged[key] = pd.concat(non_empty, axis=0, ignore_index=True) if non_empty else pd.DataFrame()
    merged["refit_models"] = {**o1.get("refit_models", {}), **o2.get("refit_models", {})}
    return merged


# Evaluation helper (identical to 06_02_01)
...
# Save helpers (identical pattern to 06_02_01)
...
def run_task(
    ...
    print("=" * 80)
    print(f"Running 06_02_02_spectral_radiomics_signature — task: {task}")
    ...
    # Cohort-1 only for training
    df_c1 = df[df["cohort"] == COHORT_1].reset_index(drop=True)
    group_lookup = build_group_lookup(df_c1, run_cfg)

    # ---- Pass 1: per-map models ----
    p1_specs_raw = get_pass1_specs(task, run_cfg)
    p1_specs = resolve_model_specs(p1_specs_raw, group_lookup)

    outputs_p1 = run_cv_for_specs(
        df=df_c1, specs=p1_specs,
    ...
    # ---- Pass 2: multi-score stacking ----
    p2_specs_raw = get_pass2_specs(task, run_cfg)
    p2_specs = resolve_model_specs(p2_specs_raw, group_lookup)

    outputs_p2 = run_cv_for_specs(
        df=df_c1, specs=p2_specs,
        train_split_value="train", test_split_value="test",
        n_splits_outer=run_cfg.cv.n_splits_outer, n_repeats_outer=n_rep,
        n_splits_inner=run_cfg.cv.n_splits_inner, random_state=run_cfg.cv.random_state,
        l1_ratios=run_cfg.enet.l1_ratios, logistic_cs=run_cfg.enet.logistic_cs,
    ...
    outputs = _merge_outputs(outputs_p1, outputs_p2)
    all_specs = p1_specs + p2_specs
    n_features_by_model = {s.name: s.n_features for s in all_specs}

    if save_outputs:
        _save_oof_outputs(outputs, task=task, task_kind=task_kind,
                          run_cfg=run_cfg, n_boot=n_boot, all_specs=all_specs)

    # ---- Cohort-1 test evaluation ----
    test_pred = outputs.get("test_predictions", pd.DataFrame())
    if not test_pred.empty:
        test_eval = _evaluate_predictions(test_pred.copy(), task_kind,
                                           n_features_by_model, run_cfg, n_boot)
        ...
        if save_outputs:
            ...
            if task_kind == "classification":
                generate_classification_plots(
                    ...
                    plots_dir=run_cfg.paths.plots_dir, task_name=task, key_models=None,
    ...
    if not df_c2.empty:
        ...
        for spec in all_specs:
            ...
            try:
                ...
                thr = (fitted.get("selected_threshold", run_cfg.cv.classification_threshold)
