...
from cv import run_cv_for_task
from cv_base import _build_pred_df
from models_base import predict_model
from stats import (
...
# Evaluation (shared pattern with 06_02 and 06_03)
def _evaluate_predictions(
    ...
    """Compute full evaluation suite for a prediction table.
    Returns dict with test_metrics, bootstrap, calibration, DCA entries.
    """
    outputs: Dict = {}

    if task_kind == "classification":
        # Finalise threshold + pred_label per model
        ...
        cal = evaluate_calibration(
            df_pred, n_bins=run_cfg.calibration.n_bins,
        ...
        dca_boot_raw = bootstrap_decision_curves(
            df_pred, thresholds=thr_grid, n_boot=n_boot,
            random_state=run_cfg.cv.random_state,
        )
...
# Saving helpers (shared pattern with 06_02 and 06_03)
def _save_outputs(
    outputs: Dict,
    *,
    task: str,
...
def _save_oof_outputs(
    ...
    *,
    ...
    if not oof_agg.empty:
        save_df(
            oof_agg,
            paths.predictions_dir / f"train_oof_{kind_tag}_predictions_{task}.csv",
            dec,
        )
    ...
    # OOF metrics xlsx — compute from aggregated predictions
    if not oof_agg.empty:
        specs = get_model_specs(task=task, run_cfg=run_cfg)
        n_feat = {s.name: s.n_features for s in specs}
        if task_kind == "classification":
            rows = []
            for mn, g in oof_agg.groupby("model_name"):
                thr = (
                    ...
                    and g["selected_threshold"].notna().any()
                    else run_cfg.cv.classification_threshold
                )
                m = classification_metrics(
                    g["target"].to_numpy(), g["pred_proba"].to_numpy(), thr
                )
                m["n_features"] = n_feat.get(mn, 1)
                rows.append({"model_name": mn, **m})
            pt = pd.DataFrame(rows)
            boot = bootstrap_classification_metrics(
                oof_agg, n_boot=n_boot,
                threshold=run_cfg.cv.classification_threshold,
                random_state=run_cfg.cv.random_state,
            )
            ...
            pt = pd.DataFrame(rows)
            boot = bootstrap_regression_metrics(
                oof_agg, n_boot=n_boot, n_features_by_model=n_feat,
                random_state=run_cfg.cv.random_state,
            )
            merged = pt.merge(boot.summary, on="model_name", how="left")
...
# Plot dispatch helper (shared pattern)
def _generate_plots(
    outputs: Dict,
    *,
    task_kind: str,
    ...
    if task_kind == "classification":
        generate_classification_plots(
            ...
            calibration_curve=outputs.get("test_calibration_curve", pd.DataFrame()),
            decision_curve=outputs.get("test_decision_curve", pd.DataFrame()),
...
def run_task(
    ...
    print("=" * 80)
    print(f"Running 06_01_conventional_ct — task: {task}")

    # ---- Build unified dataset (cohort1 + cohort2) ----
    df = build_dataset(task=task, run_cfg=run_cfg, save_csv=True)
    print(f"  Dataset shape: {df.shape}")
    print(f"  cohort1 train: {int(((df['cohort']==COHORT_1) & (df['split']=='train')).sum())}")
    print(f"  cohort1 test:  {int(((df['cohort']==COHORT_1) & (df['split']=='test')).sum())}")
    print(f"  cohort2 test:  {int((df['cohort']==COHORT_2).sum())}")

    # ---- CV + refit on cohort1 only ----
    df_c1 = df[df["cohort"] == COHORT_1].reset_index(drop=True)
    outputs = run_cv_for_task(df=df_c1, task=task, run_cfg=run_cfg)
    ...
    if save_outputs:
        _save_oof_outputs(
            outputs, task=task, task_kind=task_kind, run_cfg=run_cfg, n_boot=n_boot
        )

    # ---- Evaluate cohort-1 test set ----
    test_pred = outputs.get("test_predictions", pd.DataFrame())
    if test_pred.empty:
        print(f"  No test predictions for task {task}. Skipping test evaluation.")
        return outputs
    ...
    if save_outputs:
        ...
        for mn, fitted in outputs.get("refit_models", {}).items():
            ...
            save_pickle(
                fitted,
                run_cfg.paths.models_dir / kind_tag / f"{task}__{mn}.joblib",
            )

        _generate_plots(
            outputs, task_kind=task_kind, task_name=task,
            run_cfg=run_cfg, coefficients=outputs.get("coefficients", pd.DataFrame()),
        )

    print(f"  Finished cohort-1 evaluation for task: {task}")

    # ---- Cohort-2 evaluation (same pattern as 06_02 and 06_03) ----
    df_c2 = df[df["cohort"] == COHORT_2].reset_index(drop=True)
    if df_c2.empty:
        print(f"  No cohort-2 data available for task: {task}")
        return outputs

    print(f"  Running cohort-2 evaluation for task: {task}")
    ...
    for spec in specs:
        ...
        try:
            ...
            thr = (
                ...
                if task_kind == "classification" else np.nan
            )
    ...
    if pred_tables:
        ...
        if save_outputs:
            _save_outputs(
                c2_eval, task=task, task_kind=task_kind,
                postfix="_cohort_2", run_cfg=run_cfg,
            )
            _generate_plots(
                c2_eval, task_kind=task_kind, task_name=f"{task}_cohort_2",
                run_cfg=run_cfg,
                coefficients=outputs.get("coefficients", pd.DataFrame()),
            )

    print(f"  Finished cohort-2 evaluation for task: {task}")
    return outputs
