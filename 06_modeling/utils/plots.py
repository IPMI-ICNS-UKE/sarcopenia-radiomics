...
def plot_calibration_curves(
    ...
    for model_name, g in df.groupby("model_name"):
        ...
        ax.plot(g["pred_mean"].to_numpy(), g["obs_rate"].to_numpy(),
...
def plot_calibration_curves_interactive(
    ...
    for model_name, g in df.groupby("model_name"):
        ...
        fig.add_trace(go.Scatter(
            x=g["pred_mean"], y=g["obs_rate"], mode="lines+markers", name=model_name,
            customdata=custom,
            hovertemplate=(
...
def plot_decision_curves(
    ...
    df = df_curve[df_curve["model_name"].isin(keep)].copy() if model_names is not None else df_curve.copy()
    if df.empty:
        return
    fig, ax = plt.subplots(figsize=(8, 6))
    for model_name, g in df.groupby("model_name"):
        g = g.sort_values("threshold")
        ax.plot(g["threshold"].to_numpy(), g[metric_col].to_numpy(),
...
def plot_decision_curves_interactive(
    ...
    for model_name, g in df.groupby("model_name"):
        g = g.sort_values("threshold")
        fig.add_trace(go.Scatter(
            x=g["threshold"], y=g[metric_col], mode="lines", name=model_name,
...
def plot_probability_distribution(
    ...
    n_models = len(models)
    fig, axes = plt.subplots(nrows=n_models, ncols=1,
                             figsize=(10, max(3, 3 * n_models)), squeeze=False)
...
def plot_probability_distribution_interactive(
    ...
    for row_idx, model_name in enumerate(models, start=1):
        ...
        x = np.arange(len(g))
        colors = ["red" if t == 1 else "steelblue" for t in g["target"]]
        patient_ids = g["patient_id"].astype(str).tolist() if "patient_id" in g.columns else x.tolist()
        fig.add_trace(go.Bar(
            x=x, y=g["pred_proba"], marker_color=colors,
        ...
        if "selected_threshold" in g.columns and g["selected_threshold"].notna().any():
            thr = float(g["selected_threshold"].dropna().iloc[0])
            fig.add_hline(y=thr, line_dash="dash", line_color="black", row=row_idx, col=1,
                          annotation_text=f"Thr={thr:.3f}", annotation_position="right")
...
def plot_prediction_scatter(
    ...
    n_models = len(models)
    fig, axes = plt.subplots(nrows=n_models, ncols=1,
                             figsize=(6, max(3, 3 * n_models)), squeeze=False)
    for ax, model_name in zip(axes[:, 0], models):
...
def generate_classification_plots(
    ...
    plot_roc_curves(test_predictions, roc_dir / f"roc_{task_name}.png",
                    f"ROC — {task_name}", key_models)
    plot_calibration_curves(calibration_curve, cal_dir / f"calibration_{task_name}.png",
                            f"Calibration — {task_name}", key_models)
    plot_decision_curves(decision_curve, dca_dir / f"decision_curve_{task_name}.png",
                         f"Decision curve — {task_name}", key_models, False, dca_xlim)
    plot_probability_distribution(test_predictions, prob_dir / f"prob_dist_{task_name}.png",
                                  f"Probability distribution — {task_name}", key_models)

    if interactive_plots_dir is not None:
        plot_roc_curves_interactive(test_predictions,
                                    interactive_plots_dir / f"roc_{task_name}.html",
                                    f"ROC — {task_name}", key_models)
        plot_calibration_curves_interactive(calibration_curve,
                                            interactive_plots_dir / f"calibration_{task_name}.html",
                                            f"Calibration — {task_name}", key_models)
        plot_decision_curves_interactive(decision_curve,
                                         interactive_plots_dir / f"decision_curve_{task_name}.html",
                                         f"Decision curve — {task_name}", key_models, False, dca_xlim)
        plot_probability_distribution_interactive(test_predictions,
                                                  interactive_plots_dir / f"prob_dist_{task_name}.html",
                                                  f"Probability distribution — {task_name}", key_models)

    if coefficients is not None and not coefficients.empty:
        models_to_plot = key_models if key_models is not None else coefficients["model_name"].unique().tolist()
            ...
            if "stage" in sub.columns and sub["stage"].nunique() > 1:
                for stage in sorted(sub["stage"].dropna().unique()):
                    plot_coefficients(coefficients,
                                      ...
                                      f"Coefficients — {task_name} — {mn} — {stage}", mn, stage)
            else:
                plot_coefficients(coefficients,
                                  coef_dir / f"coef_{task_name}_{mn}.png",
                                  f"Coefficients — {task_name} — {mn}", mn, None)
...
def generate_regression_plots(
    ...
    plot_prediction_scatter(test_predictions, scatter_dir / f"scatter_{task_name}.png",
                            f"Observed vs predicted — {task_name}", key_models)
